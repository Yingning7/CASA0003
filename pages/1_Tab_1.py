...
with left:
    ...
    selected_group = st.selectbox('Grouper', ['All', 'Primary Type', 'Secondary Type'])
    if selected_group == 'Primary Type':
        selected_type = st.selectbox('Primary Type', sorted(data['primary_type'].unique().tolist()))
    elif selected_group == 'Secondary Type':
        selected_type = st.select('Secondary Type', sorted(data['secondary_type'].unique().tolist()))
    elif selected_group == 'All':
        selected_type = 'All'

print(selected_year, selected_group, selected_type)

# data_filtered = data.copy()
# data_filtered = data_filtered.loc[data['year'] == selected_year]
# data_filtered = data_filtered.loc[data_filtered[selected_group] == selected_type]
#
